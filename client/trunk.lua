...
local function NotifyOX(msg, typee)
    lib.notify({
        title = msg,
        type = typee
    })
...
RegisterNetEvent('qb-trunk:client:KidnapTrunk', function()
    ...
    if distance ~= -1 and distance < 2 then
        if isKidnapping then
            ...
            NotifyOX(_U('not_kidnapped'), 'error')
        end
    end
end)
                ...
                if not isBusy then
                    if not isKidnapped then
                        if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                            ...
                            NotifyOX(_U('entered_trunk'), 'success')
                            TrunkCam(true)
                            isKidnapped = true
                        else
                            NotifyOX(_U('trunk_closed'), 'error')
                        end
                    else
                        local vehicle = GetEntityAttachedTo(cache.ped)
                    ...
                    NotifyOX(_U('someone_in_trunk'), 'error')
                end
            else
                NotifyOX(_U('already_in_trunk'), 'error')
            end
        else
            NotifyOX(_U('cant_enter_trunk'), 'error')
        end
    else
        NotifyOX(_U('cant_enter_trunk'), 'error')
    end
...
RegisterNetEvent('qb-trunk:client:GetIn', function()
    ...
    local closestVehicle = ESX.Game.GetClosestVehicle(GetEntityCoords(cache.ped))
    if closestVehicle ~= 0 then
        local vehClass = GetVehicleClass(closestVehicle)
        local plate = GetVehicleNumberPlateText(closestVehicle)
        if Config.TrunkClasses[vehClass].allowed then
        ...
        NotifyOX(_U('no_vehicle_found'), 'error')
    end
end)
