...
local function Notify(message, typee)
	lib.notify({
		title = message,
		type = typee
	})
...
function ToggleClothing(whic, extra)
	...
	if which ~= "Mask" then
		if not Gender then Notify(Config.Notif['wrong_ped'], 'error') return false end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		...
		Notify(Config.Notif['no_variants'], 'error') return
	else
		if not LastEquipped[which] then
	...
	Notify(Config.Notif['already_wearing'], 'inform') return false
end
...
function ToggleProps(whic)
	...
	if not Prop.Variants then
		if Cur.Prop ~= -1 then -- If we currently are wearing this prop, remove it and save the one we were wearing into the LastEquipped table.
			...
			if Last then
				PlayToggleEmote(Prop.Emote.On, function() SetPedPropIndex(cache.ped, Prop.Prop, Last.Prop, Last.Texture, true) end) LastEquipped[which] = false return true
			end
		end
		Notify(Config.Notif['nothing_to_remove'], 'error') return false
	else
		local Gender = IsMpPed(cache.ped)
		if not Gender then Notify(Config.Notif['wrong_ped'], 'error') return false end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
		...
		Notify(Config.Notif['no_variants'], 'error') return false
	end
end
